...
destroy_srv_handle (struct GNUNET_MESSENGER_SrvHandle *handle)
{
  ...
  GNUNET_CONTAINER_multihashmap_iterate (handle->next_ids, iterate_free_member_ids, NULL);
                             ...
                             struct GNUNET_MESSENGER_MemberSession *session)
{
  ...
  const struct GNUNET_IDENTITY_PublicKey *pubkey = get_srv_handle_key(init->handle);
  ...
  GNUNET_log (GNUNET_ERROR_TYPE_INFO, "Initialitation: Matching member found (%s)!\n",
              GNUNET_sh2s(id));
...
static void
initialize_srv_handle_via_matching_member (struct GNUNET_MESSENGER_SrvHandle *handle,
                                           const struct GNUNET_HashCode *key)
{
  ...
  GNUNET_log (GNUNET_ERROR_TYPE_DEBUG, "Initialize member id of handle via matching member in room!\n");
                       ...
                       const struct GNUNET_HashCode *key)
{
  ...
  GNUNET_CONTAINER_multihashmap_get_multiple (handle->next_ids, key, iterate_free_member_ids, NULL);
  GNUNET_CONTAINER_multihashmap_remove_all (handle->next_ids, key);

  if ((handle->notify) && (0 == GNUNET_CONTAINER_multihashmap_size (handle->next_ids)))
  {
    GNUNET_SCHEDULER_cancel(handle->notify);
    handle->notify = NULL;
  }
                         ...
                         const struct GNUNET_MESSENGER_Message *message)
{
  ...
  {
    GNUNET_log(GNUNET_ERROR_TYPE_WARNING, "Member id does not match with handle!\n");
    return GNUNET_NO;
  }
...
get_next_member_session_context(const struct GNUNET_MESSENGER_MemberSession *session)
{
  if (session->next)
                           ...
                           const struct GNUNET_HashCode *hash)
{
  ...
  {
    const struct GNUNET_IDENTITY_PublicKey *pubkey = get_contact_key(session->member->contact);
...
task_notify_srv_handle_member_id (void *cls)
{
  ...
  GNUNET_CONTAINER_multihashmap_iterate (handle->next_ids, iterate_next_member_ids, handle);
                             ...
                             enum GNUNET_GenericReturnValue reset)
{
  ...
  struct GNUNET_MESSENGER_NextMemberId *next = GNUNET_new (struct GNUNET_MESSENGER_NextMemberId);
  if (!next)
  {
    return;
  }
  ...
  struct GNUNET_MESSENGER_NextMemberId *prev = GNUNET_CONTAINER_multihashmap_get(handle->next_ids, key);
  if (GNUNET_YES != GNUNET_CONTAINER_multihashmap_put(handle->next_ids, key, next, GNUNET_CONTAINER_MULTIHASHMAPOPTION_REPLACE))
  {
    return;
  }
  ...
  if (!handle->notify)
    handle->notify = GNUNET_SCHEDULER_add_now (task_notify_srv_handle_member_id, handle);
}
