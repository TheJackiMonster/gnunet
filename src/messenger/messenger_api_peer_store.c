...
iterate_destroy_peers (void *cls, const struct GNUNET_ShortHashCode *id, void *value)
{
...
clear_peer_store (struct GNUNET_MESSENGER_PeerStore *store)
{
  ...
  GNUNET_CONTAINER_multishortmap_iterate (store->peers, iterate_destroy_peers, NULL);
...
static enum GNUNET_GenericReturnValue
verify_store_peer(void *cls, const struct GNUNET_ShortHashCode *id, void *value)
{
  struct GNUNET_MESSENGER_ClosureVerifyPeer *verify = cls;
  struct GNUNET_PeerIdentity* peer = value;

  if ((peer) && (GNUNET_OK == verify_message_by_peer (verify->message, verify->hash, peer)))
  {
    verify->sender = peer;
    return GNUNET_NO;
  }
                   ...
                   const struct GNUNET_HashCode *hash)
{
  ...
  if ((GNUNET_MESSENGER_KIND_PEER == message->header.kind) &&
      (GNUNET_OK == verify_message_by_peer (message, hash, &(message->body.peer.peer))))
  {
    ...
    if (0 == GNUNET_memcmp (&peer_id, &(message->header.sender_id)))
      ...
      GNUNET_log (GNUNET_ERROR_TYPE_WARNING, "Sender id does not match peer identity\n");
  }
  ...
  GNUNET_CONTAINER_multishortmap_get_multiple (store->peers, &(message->header.sender_id),
                   ...
                   const struct GNUNET_PeerIdentity* peer)
{
  ...
  struct GNUNET_PeerIdentity* copy = GNUNET_memdup (peer, sizeof (struct GNUNET_PeerIdentity));
                   ...
                   const struct GNUNET_PeerIdentity *peer)
{
  ...
  if (GNUNET_YES == GNUNET_CONTAINER_multishortmap_remove (store->peers, &peer_id, find.match))
